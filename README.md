...
- **不限 vs 限量**：`product.stock` 为 NULL 表示不限库存——不扣减、永不售罄；升级到这一版后所有存量商品都是不限，需要限量的商品由管理员在「商品」页编辑时填库存数（留空即不限，填 0 即售罄）。
- **预占与归还**：下单（创建待支付订单）时原子扣减库存，不足则下单失败提示「商品库存不足」；订单取消或超时过期时加回；支付成功时预占转为实售。每张订单上记有 `stock_hold`（NONE/HELD/RELEASED/CONSUMED），归还与成交都按它做条件更新，取消、过期、付款并发时不会重复加减。
- **前台展示**：售罄商品仍在列表里，购买按钮变为「售罄」不可点；剩余 1～5 件时显示「仅剩 N 件」，其余不露具体数字。
- **三处已知取舍**：① 管理员编辑商品是「把库存设为 N」的绝对写入，若与买家下单几乎同时发生，会覆盖那次预占，偏差最多几件；② 订单先被取消/过期归还、随后 Stripe 又通知付款成功（极端竞态）时，钱已收到、订单照常成立并补扣库存，库存可能变成负数——前台按售罄处理，管理端库存列显示为「售罄（-N）」，请人工核对；③ 超时订单的库存归还是「懒惰」的：只在该买家下次打开订单列表/详情或再次进入支付页时才执行，没有定时任务；买家弃单不回来，那几件会一直处于预占状态，前台对其他人显示售罄。限量商品出现莫名售罄时，管理员在「商品」页直接把库存数改回去即可（绝对写入）。

## 商品销量

商城里每件商品显示一个「已售 N」，这个数是**两部分相加**：

- **展示销量**（`product.display_sales`）：管理员在「商品」页编辑时手填的基数，不随订单变化，默认 0。
- **实际销量**（`product.sold_count`）：订单首次入账（置 PAID）时在同一事务里按购买数量累加，只增不减（系统没有退款流程）；升级到这一版时会按已付款/已完成订单一次性回填。
- **前台口径**：接口只下发两者之和 `salesCount`，不单独暴露哪部分是手填的；总数为 0 时前台不显示「已售」。管理端列表的「销量」列同时给出总数与「实际 / 展示」拆分，编辑弹窗里实际销量只读。
...
| `mise run lint-frontend` / `lint-admin` | 商城前端 / 管理端静态检查（ESLint，警告也算失败） |
| `mise run format-frontend` / `format-admin` | 商城前端 / 管理端 Prettier 格式化；`--check` 只校验不改写（CI 用） |
...
| `POST /api/orders` | 需登录 | 创建待支付订单，body `{productId, quantity}`；库存不足返回业务码 210009 |
...
- **CI**：push main / PR 触发 `CI Backend` / `CI Frontend` / `CI Admin`（按目录过滤），复用 `Quality *` 质量门禁（后端 `mvn test`；两个前端各自 ESLint + Prettier 检查 + 单测 + 类型检查构建）。
